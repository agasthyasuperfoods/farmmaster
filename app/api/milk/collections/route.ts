    ...
    try {
      ...
      if (pageParam || limitParam) {
        ...
        const [total, records] = await Promise.all([
          ...
          MilkCollection.find(query).sort({ createdAt: -1 }).skip(skip).limit(limit)
        ]);
        ...
        }, 'MilkCollection fetched successfully');
      }

      const records = await MilkCollection.find(query).sort({ createdAt: -1 });
